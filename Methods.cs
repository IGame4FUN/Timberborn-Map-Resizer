...
namespace TB_Map_Resizer {
    class Methods
    {
        public string[] Resize_Map(string[] terrain_map, int map_size_x, int map_size_y, int map2_size_x, int map2_size_y, int shift_x, int shift_y, string desired_string)
		{   //this thing is somewhat complicated but fairly efficient, petname: Hellhole #1
			...
			Array.Fill(Resized_TerrainMap, desired_string);
			...
			for (int i = 0; i < command.Length; i++)
			{
				...
				if (command[i][0] == '-')
				{
					switch (command[i])
					{
						...
						case "-re":
							Console.WriteLine("Entities Remove True");
						...
						case "-wm":
							Console.WriteLine("Water Maps True");
							wm = true;
							break;
